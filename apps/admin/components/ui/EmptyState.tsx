...
export type EmptyStateTone = 'neutral' | 'accent' | 'statusOverdue' | 'statusPaid' | 'statusNeedsReview';
...
const TONE_CLASSES: Record<EmptyStateTone, string> = {
  ...
  statusPaid: 'bg-light-statusPaid/10 text-light-statusPaid dark:bg-dark-statusPaid/10 dark:text-dark-statusPaid',
  statusNeedsReview:
...
export function EmptyState({ icon, title, description, tone = 'neutral', action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center gap-3 py-12 text-center">
      <div className={`flex h-14 w-14 items-center justify-center rounded-full ${TONE_CLASSES[tone]}`}>
        {icon}
      </div>
      <h2 className="text-base font-semibold text-light-textPrimary dark:text-dark-textPrimary">{title}</h2>
      {description ? (
        <p className="max-w-sm text-sm text-light-textSecondary dark:text-dark-textSecondary">{description}</p>
      ) : null}
