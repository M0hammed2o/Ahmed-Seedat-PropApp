...
// Provisioned-staff account model (this date): the "Add staff member" form, POSTing to
// /staff-provisions instead of the legacy /invites (see StaffAccessPanel.tsx's own comment on why
// both routes coexist). Deliberately near-identical to InviteStaffForm.tsx -- same field shape,
// same property-access UI -- because this is a different provisioning MECHANISM, not a different
// data model.
...
export interface AddStaffMemberResult {
  ...
  emailOutcome: 'activation_sent' | 'notification_sent' | 'send_failed';
}
...
export function AddStaffMemberForm({
  ...
  /** Mirrors the legacy MAX_INVITABLE_ROLE_FOR ceiling client-side only, for a tighter dropdown --
   * provision_staff_member() re-enforces this itself regardless of what this form ever offers. */
  invitableRoles: OrganizationMemberRole[];
  onAdded: (result: AddStaffMemberResult) => Promise<void>;
  onCancel: () => void;
}) {
  ...
  async function submit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/api/v1/organizations/${orgId}/staff-provisions`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        setError(body.error?.message ?? 'Failed to add staff member.');
        return;
      }
      await onAdded({
        isExistingActiveUser: body.isExistingActiveUser ?? false,
        emailDeliveryConfigured: body.emailDeliveryConfigured ?? null,
        emailOutcome: body.emailOutcome ?? 'send_failed',
      });
    } finally {
      setSubmitting(false);
    }
