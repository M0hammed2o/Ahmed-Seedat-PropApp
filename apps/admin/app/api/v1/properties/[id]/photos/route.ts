...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  // narrower attack surface than the document-upload paths scanUpload()'s default now
  ...
  // safety improvement -- see lib/uploadScan.ts's own comment for the full reasoning.
  const scan = await scanUpload(buffer, { sensitive: false });
  ...
  // Written by the server after the agent+/property-access checks above: clients have no Storage
  // write policy (migration 20260101000171).
  ...
  // Resized copies are generated by the server from the scanned original, not supplied by the user.
  if (derivatives.hero && heroStoragePath) {
    const heroStored = await storeServerGeneratedObject({
      orgId: property.org_id,
      path: heroStoragePath,
      bytes: derivatives.hero.buffer,
      contentType: derivatives.hero.contentType,
    });
    if (heroStored.ok) uploadedDerivativePaths.push(heroStoragePath);
  }
  if (derivatives.card && cardStoragePath) {
    const cardStored = await storeServerGeneratedObject({
      orgId: property.org_id,
      path: cardStoragePath,
      bytes: derivatives.card.buffer,
      contentType: derivatives.card.contentType,
    });
    if (cardStored.ok) uploadedDerivativePaths.push(cardStoragePath);
  }
  ...
  const { data: documentRow, error: documentError } = await supabase
    ...
    .single();
  if (documentError) {
    await removeProtectedObjects([storagePath, ...uploadedDerivativePaths]);
    return NextResponse.json(
