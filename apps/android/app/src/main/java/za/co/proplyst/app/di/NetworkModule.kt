...
import za.co.proplyst.app.data.auth.TokenAuthenticator
...
object NetworkModule {
    ...
    fun provideSupabaseOkHttpClient(
        ...
        return OkHttpClient.Builder()
            ...
            // Android V1 final gap-closure pass, Phase 2: automatic access-token refresh on a
            // real 401, not a pre-emptive check on every call -- see TokenAuthenticator's own
            // header comment for why this needs to be an Authenticator, not another Interceptor.
    ...
    fun provideWebOkHttpClient(
        ...
        return OkHttpClient.Builder()
            .addInterceptor(authInterceptor(sessionManager))
            .addInterceptor(logging)
            // Same TokenAuthenticator singleton as the Supabase client -- one session, one
            // refresh path, regardless of which host a 401 comes from.
            .authenticator(tokenAuthenticator)
            .build()
    }
