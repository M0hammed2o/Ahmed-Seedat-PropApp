...
## 9. Known current gaps (carried forward, not fixed)

These are real, current, and unresolved as of the last audit (`EXISTING_CODEBASE_AUDIT.md` §8) — this rewrite does not claim either is fixed:

- **`jest-expo`/Windows test-runner bug** (`KNOWN_BUGS.md`): every `apps/mobile` Jest suite — including pure-logic files with zero Expo imports — fails at collection time with `TypeError: The "path" argument must be of type string. Received null` inside `jest-expo/src/preset/setup.js`. Root-caused to `jest-expo@56.0.5`'s stack-trace-based module lookup misbehaving on this Node/Jest/Windows combination; confirmed not a stale-dependency or MAX_PATH issue. Unblocking paths (macOS/Linux/WSL, a `jest-expo` release past `56.0.5`, or a manual RN Jest config dropping the preset) are documented in `KNOWN_BUGS.md` and remain unattempted.
- **Expo Router typed-route typecheck failure** (found during the audit, not previously in `KNOWN_BUGS.md`): `pnpm typecheck` fails for `apps/mobile` with 4 `TS2322` errors (`properties/[id]/processing.tsx`, `review.tsx` ×2, `upload.tsx`) — route-string literals not matching Expo Router's generated typed-route union. Real, reproducible, still open.
- **Both become moot, not fixed, once `apps/mobile` is retired.** Per `MOBILE_ARCHITECTURE_DECISION.md`, `apps/mobile` is reference-only during the native migration and is never deployed to app stores; `apps/ios`/`apps/android` (§6 of this document) are the native mobile test surface going forward. Until that retirement actually happens, `apps/mobile`'s test/typecheck failures remain open items — they should not be reported as resolved, and CI's mobile job status should continue to reflect the actual failing state rather than being silently green-washed by exclusion.
- **RLS test execution environment.** The existing `supabase/tests/rls_isolation.test.sql` was written but its execution was Blocked in the environment that authored it (no local Docker/Supabase instance available there — `KNOWN_BUGS.md`). §2 of this document assumes a CI runner or local dev machine with Docker available to actually run `supabase test db`; if that's not yet wired into CI, it is the single highest-priority test-infra gap to close given §2's non-negotiable status, and should be tracked explicitly rather than assumed done because the `.sql` files exist.
