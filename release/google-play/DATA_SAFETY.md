...
The session token is held in `EncryptedSharedPreferences` (`androidx.security.crypto`).
`android:allowBackup="false"` keeps it out of Android auto-backup archives, and
`android:dataExtractionRules` (`res/xml/data_extraction_rules.xml`) keeps all app data out of
Android 12+ device-to-device transfer too, so a token cannot be restored onto a different device. If
the encrypted store ever cannot be read on this device, it is wiped and the user signs in again.

Signing out ends only that device's session (`auth/v1/logout?scope=local`).

The Privacy Policy and Terms of Service are linked inside the app: More → Legal (tenant accounts:
Profile).
...
| `za.co.genbridge.proplyst.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` | Generated by AndroidX, internal to the app itself | No — not a user-facing permission |

**That is all four.** No `CAMERA`, no `READ_MEDIA_IMAGES`/`READ_EXTERNAL_STORAGE`, no
