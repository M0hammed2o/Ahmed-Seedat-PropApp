...
Enforced at three layers: (1) the posting service is the only code path allowed to write `journal_entries`/`journal_lines` (`ARCHITECTURE.md`); (2) RLS has no `update`/`delete` policy on those tables for any role, including elevated ones (`DATABASE.md` §12); (3) the posting service itself validates `SUM(debit) = SUM(credit)` before any insert and rejects the whole entry (not a partial post) if unbalanced.
