...
Active/Approved in Meta, and provided the real approved parameter structure for 7 of the 8 —
every call site below was rewritten to match exactly. `rent_payment_reminder` was explicitly
excluded from that hand-off ("verify against current implementation... do not guess if not
already documented") and remains this codebase's own unverified guess
...
| `rent_payment_reminder`          | Tenant                                          | `rent_schedules_due_soon()` sweep, daily-jobs                                        | organizationName, amount, dueDate — **UNVERIFIED, not part of the 2026-08-17 hand-off**                                                                       | `rent`                 |
