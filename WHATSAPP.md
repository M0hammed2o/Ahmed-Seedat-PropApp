...
trigger, and variable order (`lib/whatsappTemplates.ts`'s `expectedVariableCount`). **Updated
2026-08-17**: Mohammed directly confirmed, in conversation, that all 8 templates show
Active/Approved in Meta. He first hand-provided the real approved parameter structure for 7 of
the 8 (`rent_payment_reminder` explicitly excluded, "verify against current implementation...
do not guess"), then the SAME DAY exported and reviewed the actual approved body text for all 8
directly from Meta WhatsApp Manager — including `rent_payment_reminder`, correcting the earlier
unverified 3-var guess (`organizationName, amount, dueDate`) to its real 5-var structure. **All 8
templates are now `variableCountVerified: true`.** Every call site builds its `variables` object
through a matching function in `lib/whatsappTemplateVariables.ts` (never an inline object
literal), directly unit-tested per template in `whatsappTemplateVariables.test.ts`.

| Template                         | Recipient                                       | Real trigger                                                                         | Variables (real approved order, confirmed 2026-08-17 unless noted)                                                                                            | Category               |
| -------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `tenant_account_invitation`      | Tenant                                          | Staff clicks "Send invitation"                                                       | organizationName, acceptUrl, supportName                                                                                                                      | (none — transactional) |
| `payment_received_confirmation`  | Tenant                                          | Staff/RPC confirms a payment report, or a cash receipt/bank transaction is confirmed | amount, propertyLabel, paymentPeriod, dateConfirmed, accountLink                                                                                              | `rent`                 |
| `payment_confirmation_required`  | Every linked, contactable owner (independently) | A tenant or staff member reports a new payment                                       | amount, propertyLabel, tenantName, paymentMethod, paymentPeriod, reviewLink                                                                                   | `rent`                 |
| `rent_payment_reminder`          | Tenant                                          | `rent_schedules_due_soon()` sweep, daily-jobs                                        | amount, paymentPeriod, dueDate, propertyLabel, accountLink                                                                                                    | `rent`                 |
| `rent_overdue_notice`            | Tenant                                          | `rent_schedules_overdue_unreminded()` sweep, daily-jobs                              | outstandingAmount, tenantName, propertyLabel, paymentPeriod, accountLink                                                                                      | `rent`                 |
| `maintenance_request_update`     | Tenant                                          | A maintenance ticket's status changes (urgent priority only)                         | propertyLabel, summary, status, updateMessage, ticketLink                                                                                                     | `maintenance`          |
| `lease_expiry_reminder`          | Tenant                                          | `leases_expiring_unreminded()` sweep, daily-jobs                                     | tenantName, propertyLabel, expiryDate, leaseLink                                                                                                              | `lease`                |
| `owner_monthly_property_summary` | Every linked, contactable owner (independently) | `runOwnerMonthlySummaryJob()`, on the owner's preferred day, retried until sent      | month, propertyCount, expectedRent, confirmedPaid, outstanding, awaitingConfirmation, openMaintenance, upcomingLeaseExpiries, reportUrl (no organizationName) | `owner_summary`        |
