...
## 2026-08-05 — PRODUCT DECISION: PWA-first commercial launch, native apps frozen; shared ownership/property permissions/owner portal are mandatory V1 scope, not V2
...
**Mohammed's follow-up instruction explicitly overrode that recommendation**: shared ownership, ownership percentages, property-level permissions, and a dedicated Owner Portal are now **mandatory V1 launch requirements**, not a V2 differentiator — stated as three named V1 customer types (Individual Owner, Shared Family Ownership, Trusts/Family Offices), with the explicit carve-out that "complex trustee approval workflows may be deferred to a future release, but shared ownership must work from Version 1." This is recorded as a decision, not silently absorbed — the honest scope implication (an approved execution plan, `~28-38` engineering-weeks total, with the property-permission rebuild alone estimated at 4-6 weeks as the single highest-risk item) was surfaced back to him before any implementation began, per this project's standing convention of naming scope growth explicitly rather than quietly building less.

**Design decision adopted for the property-permission model** (from the architecture review, not re-litigated when the follow-up instruction confirmed the six-role list verbatim): the roles Mohammed named — Owner, Property Manager, Accountant, Maintenance Manager, Administrator, Read Only — become a **new, orthogonal property-scoped axis**, layered on top of the existing org-level role (`principal`/`manager`/`agent`/`accountant`/`viewer`), not a replacement for it. A user's effective permission on a given property is their org role **and** their property-scoped role together — the same compositional pattern `has_org_role()` already uses, extended with a new `has_property_access()` primitive rather than a redesigned role system.

**Full plan**: a complete, evidence-grounded execution plan (current-state audit across all 11 of Mohammed's stated launch phases, a dependency-ordered Stage 0-8 build sequence, per-phase effort estimates, and named product decisions still requiring his input — hosting target, OCR/email/WhatsApp vendor selection, tier-gating for governance features) was written and approved via Plan Mode, saved at `C:\Users\junsm\.claude\plans\dynamic-purring-llama.md`. `TASKS.md` (see its 2026-08-05 addendum) is the pointer from the milestone list to this plan; `WORKLOG.md` this date records the first slice of actual implementation (Stage 0).
...
## 2026-08-12 (continued) — Compliance completion pass: two changes to already-shipped behaviour, both narrowly scoped

Closing the gaps this workflow's own first-pass report disclosed (full detail: `WORKLOG.md` this date) required two decisions that touch code already committed in the pass above, recorded separately since they're a materially different kind of call than "what to build next."

**`activate_property_rule_version()`'s tenant-status filter was changed, not left as documented behaviour, because it was found to be a real bug rather than a deliberate scope line.** The original `t.status = 'active'` read as a plausible intentional guard when written. Building real E2E coverage (not pgTAP, whose fixtures insert `status: 'active'` directly and so never exercised the ordinary flow) showed that no application code path outside the tenant-screening-approval flow ever sets that column — meaning the entire manual staff tenant-creation path, this codebase's own primary documented workflow, would never receive a compliance requirement. This is corrected via a new migration (`20260101000099`) rather than editing migration `097` in place, per this task's explicit "do not rewrite what's already committed" instruction; the lease's own `status = 'active'` (already the correct, existing "current tenancy" signal used elsewhere) is now sufficient on its own, with `tenants.status` checked only to exclude a record explicitly marked `expired`.

**Notification lifecycle deliberately stops short of "rules updated" and "levy ready for review" as distinct notification events**, even though the task listed both. "Rules updated" is, from a recipient's standpoint, the identical event as "requirement assigned" (a new version activating always assigns a fresh requirement) — a second, separately-worded notification for the same trigger would be redundant rather than informative. "Levy statement ready for review" would fire for the same staff member who is already watching the extraction complete synchronously in the UI they just used to upload it — notifying someone of something they're already looking at is noise, not signal. Both are one-line judgment calls, reversible if a future async/batch-upload path makes either genuinely useful.
